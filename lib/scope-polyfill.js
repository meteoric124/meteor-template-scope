...
function attach_$scope_to_element () {
    ...
    if (single_node) {
        ...
        let already_have_scope = $element.attr('uid');
        ...
        if (already_have_scope) {
            ...
            if (!currentElemScopeIsMoreSpecific) {
                $element.attr('uid', this.$uid);
                // $element.attr('scope', this.$scope.$id);
                $element.$data('$scope', this.$scope);
            }
        } else {
            $element.attr('uid', this.$uid);
            $element.attr('scope_id', this.$scope.$id);
            $element.$data('$scope', this.$scope);
        }
    }
}
...
Template.onRenderedFirst(function() {
    ...
    if (this.$isRootTemplate) {
        ...
        Meteor.setTimeout(this.$preLinkHandler.bind(this));  // Finish rendering.
    } else {
        /*
    ...
    if (is_leaf_template.call(this)) {
        if (!this.$preLinked) {
            ...
            Meteor.setTimeout(this.$postLinkHandler.bind(this));  // Finish rendering.
        }
    }
});
